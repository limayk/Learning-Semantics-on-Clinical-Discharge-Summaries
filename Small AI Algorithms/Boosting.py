...
def main():
    ...
    if False: #makes the training set 50/50 True, False
    ...
    #hyperparameters
    number_of_rounds = 5000 #Tree =1, boosting/Forest >1
    early_stopping = 500 #validation needs to improve at least every x rounds
    ...
    if False: #use olf model?
        ...
        param = {
            'max_depth': 3,           # Reduced max depth (typical for boosting)
            ...
            'num_parallel_tree': 1    # IMPORTANT: Set to 1 for boosting (>1 would make it a random forest)
        }

        #training
        evals_result = {}
        bst = xgb.train(param, dtrain, number_of_rounds, evallist, evals_result=evals_result, early_stopping_rounds=early_stopping) #validation needs to improve at least every 10 rounds

    #saving the model
    bst.save_model(f"saved_weights/{number_of_rounds}_rounds.model")

    #plotting
    make_plot(evals_result)

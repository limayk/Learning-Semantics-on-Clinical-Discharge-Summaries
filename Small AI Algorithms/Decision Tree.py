...
def main():
    ...
    #hyperparameters
    number_of_rounds = 1 #Tree =1, boosting/Forest >1
    early_stopping = None #validation needs to improve at least every x rounds
    ...
    if False: #use olf model?
        ...
        param = {
            'max_depth': 10,           # Reduced max depth (typical for boosting)
            'eta': 1,               # Learning rate (increased slightly)
            'objective': 'binary:logistic',
            'lambda': 0.1,            # L2 regularization
            'alpha': 0.0,             # L1 regularization
            'subsample': 1,         # Use 80% of data per tree to prevent overfitting
